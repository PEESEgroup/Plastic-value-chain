...
Pre_eq6(m)$(MPL(m))..      pllll(m) =e= sum(j,(rrrr(j)*C(j,m)))*PLR(m)/(PLR(m)+1);
Pre_eq7(m)$(MAAA(m))..     pllll(m) =e= (sum(j,(rrrr(j)*C(j,m)))+sum(i$(IBIO(i)),(ma(i)*CC(i,m))))*PLR(m)/(PLR(m)+1);
...
Post_eq17(m)..             wpl(m) =e= sum(j,(rrrr(j)*(1-C(j,m)))*C(j,m)/(C(j,m)+1e-3))*PLR(m)/(PLR(m)+1);
...
Econ_eq39(m)..             cpp(m) =e= (sum(j,ri(j)*C(j,m)*OPP(j,m))+sum(i$IBIO(i),ma(i)*CC(i,m)*OPJP(i,m)))*PLR(m)/(PLR(m)+1);
...
Envv_eq55(m,p)..           envp(m,p) =e= (sum(j,ri(j)*C(j,m)*ENVPP(j,m,p))+sum(i$IBIO(i),ma(i)*CC(i,m)*ENVJP(i,m,p)))*PLR(m)/(PLR(m)+1);

Envv_eq57(m,p)..           envw(m,p) =e= WA(m)*ENINV(m,p)+wasex(m)*ENLFV(m,p)+sum(n,pc(m,n)*ENMPV(m,n,p));
Envv_eq58(p)..             enva(p) =e= mm('8')*ENVCO(p)+(1-SUB)*sum(m$(MPL(m)),(RPP(m)*ENAVV(m,p))+ENAVV(m,p)*pllll(m)*PLR(m)/(PLR(m)+1))+SUB*sum(m$(MPA(m)),(RPP(m)*ENAVV(m,p))+ENAVV(m,p)*pllll(m)*PLR(m)/(PLR(m)+1));
...
execute_unload 'results_1.gdx' npvv envv zz wasee plllll
execute 'gdxxrw.exe results_1.gdx o=results_1.xlsx par=npvv rng=NewSheet1!';
execute 'gdxxrw.exe results_1.gdx o=results_1.xlsx par=envv rng=NewSheet2!';
execute 'gdxxrw.exe results_1.gdx o=results_1.xlsx par=zz rng=NewSheet3!';
execute 'gdxxrw.exe results_1.gdx o=results_1.xlsx par=wasee rng=NewSheet4!';
execute 'gdxxrw.exe results_1.gdx o=results_1.xlsx par=plllll rng=NewSheet5!';
$offText
...
loop (ss,ELEE = ELEEE(ss);
OPR('8','1') = OPR('8','1')*INFF('2',ss);
...
execute_unload 'results_4.gdx' npvvv envvv zzz waseee pllllll
execute 'gdxxrw.exe results_4.gdx o=results_4.xlsx par=npvvv rng=NewSheet1!';
execute 'gdxxrw.exe results_4.gdx o=results_4.xlsx par=envvv rng=NewSheet2!';
execute 'gdxxrw.exe results_4.gdx o=results_4.xlsx par=zzz rng=NewSheet3!';
execute 'gdxxrw.exe results_4.gdx o=results_4.xlsx par=waseee rng=NewSheet4!';
execute 'gdxxrw.exe results_4.gdx o=results_4.xlsx par=pllllll rng=NewSheet5!';
